...
{
    public class DocInfoCollection : List<string>
    {

        internal DocInfoCollection(XPathNavigator rpcResponseNav, string elementName)
        {
            if (rpcResponseNav != null)
            {
                XPathNodeIterator nodes = rpcResponseNav.Select("//ul[@name='" + elementName + "']/ul/li"); 
                while (nodes.MoveNext())
                {
                    this.Add(nodes.Current.Value);
                }
            }
        }
