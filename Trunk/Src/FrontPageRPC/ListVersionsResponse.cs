...
{
    public class ListVersionsResponse : ResponseBase
    {
        private DocumentCollection _documents;
        ListVersionsRequest _request;

        internal ListVersionsResponse(WebClient wc, ListVersionsRequest request)
            : base(wc)
        {
            _request = request;
        }
        ...
        internal override void ReadResponse(Stream response, long responseLength)
        {
            ...
            this._documents = new DocumentCollection(base._nav, "version_list", _request.WebUrl);
        }
