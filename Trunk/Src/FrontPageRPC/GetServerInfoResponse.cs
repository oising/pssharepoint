...
namespace HubKey.Net.FrontPageRPC
{
    ...
    {
        GetServerInfoRequest _getServerInfoRequest;
        ServerInfo _serverInfo;

        internal GetServerInfoResponse(WebClient wc, GetServerInfoRequest getServerInfoRequest)
            : base(wc)
        {
            _getServerInfoRequest = getServerInfoRequest;
        }
        ...
        internal override void ReadResponse(Stream response, long responseLength)
        {
            ...
            int nread = 0;
            while ((nread = response.Read(buffer, 0, 4096)) != 0)
            {
                responseString.Append(Encoding.UTF8.GetString(buffer, 0, nread));
            }
            _serverInfo = new ServerInfo(responseString.ToString());
        }
